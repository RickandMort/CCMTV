...
public class CustomizedProgressBar2 extends View {
    ...
    public CustomizedProgressBar2(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context);
    }

    public CustomizedProgressBar2(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public CustomizedProgressBar2(Context context) {
        super(context);
        initView(context);
    }
...
//        canvas.drawRoundRect(rectBg, round, round, mPaint);//绘制 最外面的大 圆角矩形，背景为白色
        canvas.drawRoundRect(rectBg, 100,
                100, mPaint);//绘制 最外面的大 圆角矩形，背景为白色
...
//        canvas.drawRoundRect(rectProgressBg, round, round, mPaint); //最左边的圆角矩形
        canvas.drawRoundRect(rectProgressBg, 100, 100, mPaint); //最左边的圆角矩形

        if (maxCount != currentCount){ //如果不是100%，绘制第三段矩形
//            RectF rectProgressBg2 = new RectF(mWidth*section-round, 0, mWidth*section, mHeight);
            RectF rectProgressBg2 = new RectF(mWidth*section-round, 0, 2, mHeight);
            mPaint.setShader(getLinearGradient());
    ...
    private LinearGradient getLinearGradient(){
        if(linearGradient==null){
            linearGradient = new LinearGradient(0, 0, getWidth(), mHeight, new int[]{mContext.getResources().getColor(R.color.color3897f9),
                    mContext.getResources().getColor(R.color.color3897f9)}, null, Shader.TileMode.CLAMP); //根据R文件中的id获取到color
        }

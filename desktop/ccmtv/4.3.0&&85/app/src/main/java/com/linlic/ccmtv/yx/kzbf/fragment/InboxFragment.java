...
//                            Toast.makeText(getActivity(), jsonObject.getString("msg"), Toast.LENGTH_SHORT).show();
    ...
    private void setResultStatus(boolean status, int code) {
        if (status) {
            inbox_list.setVisibility(View.VISIBLE);
            ...
            inbox_list.setVisibility(View.GONE);
    ...
    private void initView() {
        inbox_list = (RecyclerView) getActivity().findViewById(R.id.inbox_list);
        lt_nodata1 = (NodataEmptyLayout) getActivity().findViewById(R.id.rl_inbox_nodata1);
    }

    private void initData() {
        inbox_list.setLayoutManager(new RecyclerViewNoBugLinearLayoutManager(getContext()));
        meAdapter = new MessageAdapter(getContext(), listMore);
        inbox_list.setAdapter(meAdapter);

        meAdapter.disableLoadMoreIfNotFullPage(inbox_list);//取消第一次进入加载 下拉加载更多方法
        ...
        meAdapter.setOnLoadMoreListener(new BaseQuickAdapter.RequestLoadMoreListener() {
            ...
            public void onLoadMoreRequested() {
                inbox_list.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        if (itemCount >= count) {
                            //数据全部加载完毕
...
//                    Log.e("看看收件箱数据", result);
    ...
    private void setBean(JSONObject jsonObject) throws JSONException {
        ...
        dbMe.setAddressor_uid(jsonObject.getString("addressor_uid"));
        dbMe.setAddtime(jsonObject.getString("addtime"));
        dbMe.setU_is_look(jsonObject.getString("u_is_look"));

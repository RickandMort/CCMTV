...
import static com.linlic.ccmtv.yx.activity.rules_to_compensate.training_management.New_teaching_activities.lecturer_data;
...
public class Event_Details_lecturer extends BaseActivity{
    private Context context;
    private String fid = "";
    private String id = "";
    private String type = "";
    boolean check_in_bool = false;
    List<Resident> lecturer_data2 = new ArrayList<>();//讲师 数据
    @Bind(R.id.selectParticipant)
    LinearLayout selectParticipant;//选择参与人员
    @Bind(R.id.lecturers)
    GridView lecturers;//讲师
    ...
    public void initView(){
        ...
        setResultStatus((type.equals("1")?lecturer_data.size():lecturer_data2.size()) > 0, 200);

        baseListAdapterLecturers = new BaseListAdapter(lecturers, type.equals("1")?lecturer_data:lecturer_data2, R.layout.item_lecturers) {
    ...
    public void Selectlecturer(){
        Intent intent = new Intent(context, Select_lecturer.class);
        intent.putExtra("fid", fid);
        Bundle bundle=new Bundle();
        bundle.putSerializable("lecturer_data", (Serializable)lecturer_data);//序列化
...
//                    LogUtil.e("返回的数据",residents.toString());
                        for (Resident resident : le) {
                            ...
                            baseListAdapterLecturers.refresh(lecturer_data);
                        setResultStatus(lecturer_data.size() > 0, 200);
                        break;
                    default:
                        break;
                }
    ...
    public void deleteresidents(View view) {
        for (int i = 0; i < lecturer_data.size(); i++) {
            Resident resident1 = lecturer_data.get(i);
            if (resident1.getId().equals(view.getTag().toString())) {
                lecturer_data.remove(resident1);

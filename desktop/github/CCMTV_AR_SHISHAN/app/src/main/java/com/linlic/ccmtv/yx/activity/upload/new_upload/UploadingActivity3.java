...
import com.linlic.ccmtv.yx.activity.upload.adapter.UploadingAdapter3;
...
public class UploadingActivity3 extends BaseActivity implements XExecutor.OnAllTaskEndListener, View.OnClickListener {

    private UploadingAdapter3 adapter;
    ...
    private void initData() {
        ...
        adapter = new UploadingAdapter3(this);
        adapter.updateData(UploadingAdapter3.TYPE_ING);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.setAdapter(adapter);
        adapter.OnAddItemClickListener(new UploadingAdapter3.OnItemClickListener() {
            @Override
            public void plusNum() {
                checkNum++;
        ...
        switch (v.getId()) {
            ...
            case R.id.id_upload_topRightTxt2://右上角编辑按钮
                ...
                if (videoValues.size() > 0) {
                    delete_video.setTextColor(Color.parseColor("#000000"));
                    for (int i = 0; i < videoValues.size(); i++) {
                        uploadTask = videoValues.get(i);
                        uploadModel = (UploadModel) uploadTask.progress.extra1;
            ...
            case R.id.id_upload_delete_video2://删除按钮
                try {
                    ...
                    for (int i = 0; i < videoValues.size(); i++) {
                        UploadModel uploadModel = (UploadModel) videoValues.get(i).progress.extra1;
                        if (uploadModel.isChecked()) {
//                            Log.e("delete", "onClick: 暂停并删除"+videoValues.get(i).progress.tag);
                            okUpload.getTask(videoValues.get(i).progress.tag).remove();
//                            okUpload.removeTask(videoValues.get(i).progress.tag);
                        }
                    }
                    upload_bottom.setVisibility(View.GONE);
                    upload_topRightTxt.setText("编辑");

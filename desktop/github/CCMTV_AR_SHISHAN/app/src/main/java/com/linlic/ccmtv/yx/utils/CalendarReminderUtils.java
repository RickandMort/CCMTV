...
public class CalendarReminderUtils {
     ...
     * 添加日历事件
     */
    public static Uri addCalendarEvent(Context context, String title, String description, long reminderTime, int previousDate) {
        if (context == null) {
            return null;
        }
...
//        values.put(CalendarContract.Reminders.MINUTES, previousDate * 24 * 60);// 提前previousDate天有提醒
        values.put(CalendarContract.Reminders.MINUTES, previousDate * 13);// 提前previousDate天有提醒
        ...
        Cursor eventCursor = context.getContentResolver().query(newEvent, null, null, null, null);
...
//        Log.e("新增日程后查询", "  eventTitle:"+eventCursor.getString(eventCursor.getColumnIndex("title"))+" id:"+eventCursor.getInt(eventCursor.getColumnIndex(CalendarContract.Calendars._ID)) );
     ...
     * 查找日历事件
     */
    public static boolean findCalendarEvent(Context context,String title,long time) {
        if (context == null) {
            return false;
        }
        Boolean bool = false;
        ...
        try {
            ...
            if (eventCursor.getCount() > 0) {
                ...
                for (eventCursor.moveToFirst(); !eventCursor.isAfterLast(); eventCursor.moveToNext()) {
                    ...
                    startCalendar.setTimeInMillis(starttime);//设置开始时间
//                       Log.e("日程", "  uid "+uid );
                    if (!TextUtils.isEmpty(title) && title.equals(eventTitle)&&
                            mCalendar.get(Calendar.YEAR) == startCalendar.get(Calendar.YEAR) && mCalendar.get(Calendar.MONTH) == startCalendar.get(Calendar.MONTH) &&
                            mCalendar.get(Calendar.DAY_OF_MONTH) == startCalendar.get(Calendar.DAY_OF_MONTH) && mCalendar.get(Calendar.HOUR_OF_DAY) == startCalendar.get(Calendar.HOUR_OF_DAY) &&
                            mCalendar.get(Calendar.MINUTE) == startCalendar.get(Calendar.MINUTE)) {
                        int id = eventCursor.getInt(eventCursor.getColumnIndex(CalendarContract.Calendars._ID));//取得id

//                        Log.e("日程","id:"+id+"  eventTitle:"+eventTitle+" starttime:"+starttime);
                        bool = true;
                    }
                }
                return bool;
     ...
     * 查找日历事件
     */
    public static void findCalendarEventAll(Context context,String title) {
        if (context == null) {
            return ;
        }
        Cursor eventCursor = context.getContentResolver().query(Uri.parse(CALENDER_EVENT_URL), null, null, null, null);

...
public class AmapActivity extends BaseActivity implements AMap.OnCameraChangeListener, AMap.OnMapLoadedListener, AMapLocationListener, PoiSearch.OnPoiSearchListener, View.OnClickListener {
...
//        mWifiManager = (WifiManager) this.getSystemService(Context.WIFI_SERVICE);
        ...
        searchResultAdapter = new SearchResultAdapter(AmapActivity.this);
    ...
    AdapterView.OnItemClickListener onItemClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            if (position != searchResultAdapter.getSelectedPosition()) {
                ...
                aMap.moveCamera(CameraUpdateFactory.changeLatLng(new LatLng(35.232031D,125.467688D)));
    ...
    public void onCameraChangeFinish(CameraPosition cameraPosition) {
        ...
        LogUtil.e("经纬度",mcircle.getCenter().toString());
        if (mcircle != null) {
            if (mcircle.contains(cameraPosition.target)){
                checkinpoint = cameraPosition.target;
            } else{
                Toast.makeText(AmapActivity.this, "微调距离不可超过500米", Toast.LENGTH_SHORT).show();
//                aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(mlocation ,16f ));
            }
        }else {
            startLocation();
            Toast.makeText(AmapActivity.this, "重新定位中。。。", Toast.LENGTH_SHORT).show();
        }
    }
...
//                aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(mlocation ,16f ));
            ...
            if (mcircle != null) {
                ...
                mcircle = aMap.addCircle(new CircleOptions().center(mlocation).radius(5).strokeWidth(5));
                mcircle = aMap.addCircle(new CircleOptions().center(new LatLng(35.232031D,125.467688D)).radius(5).strokeWidth(5));
            }
    ...
    public void onPoiSearched(PoiResult poiResult, int resultCode) {
        ...
        if (resultCode == AMapException.CODE_AMAP_SUCCESS){
            if (poiResult != null && poiResult.getPois().size() > 0){
                ...
                Toast.makeText(AmapActivity.this, "无搜索结果", Toast.LENGTH_SHORT).show();
            }
        } else {
            Toast.makeText(AmapActivity.this, "搜索失败，错误 "+resultCode, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void checkin() {
        if (checkinMarker  == null ){
            if (checkinpoint != null) {
                ...
                Toast.makeText(AmapActivity.this, "签到成功", Toast.LENGTH_SHORT).show();
            } else{
//                startLocation();
                Toast.makeText(AmapActivity.this, "签到失败。。。", Toast.LENGTH_SHORT).show();
            }
        }else {
            Toast.makeText(AmapActivity.this, "今日已签到", Toast.LENGTH_SHORT).show();
        }
    }
}

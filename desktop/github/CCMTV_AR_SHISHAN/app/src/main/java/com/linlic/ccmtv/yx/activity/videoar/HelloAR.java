...
//        for (ImageTarget target : ImageTarget.setupAll(path, StorageType.Assets)) {
    ...
    public void loadAllFromJsonFile2(  String path)
    {
    ...
    public boolean initialize()
    {
        ...
        ImageTracker tracker = new ImageTracker();
        tracker.attachStreamer(streamer);
        loadAllFromJsonFile(tracker, URLConfig.ccmtvapp_basesdcardpath + "/arImage/targets.json");
                ...
                if (status == TargetStatus.Tracked) {
                    ...
                    if (tracked_target == 0) {
                        if (video == null && video_renderers.size() > 0) {
                            ...
                            video = new ARVideo();
                            if (target_name.equals("cc") && video_renderers.get(0).texId() != 0) {
                                video.openVideoFile("ccmtv.mp4", video_renderers.get(0).texId());
                                current_video_renderer = video_renderers.get(0);
                            } else  {
                                video.openStreamingVideo(target.uid(), video_renderers.get(0).texId());
                                current_video_renderer = video_renderers.get(0);
                            }
                        }
                        if (video != null) {
